...
class ProxifyPlugin extends AbstractPlugin
{
    ...
    private function html_attr($matches)
    {
        ...
        $schemes = ['data:', 'magnet:', 'about:', 'javascript:', 'mailto:', 'tel:', 'ios-app:', 'android-app:'];
    ...
    private function proxify_head($str)
    {
        ...
        $str = preg_replace_callback('/content=(["\'])\d+\s*;\s*url=(.*?)\1/is', [$this, 'meta_refresh'], $str);
    ...
    public function onCompleted(ProxyEvent $event)
    {
        ...
        $no_proxify = ['text/javascript', 'application/javascript', 'application/x-javascript', 'text/plain'];
        ...
        // src= and href=
        $str = preg_replace_callback('@(?:src|href)\s*=\s*(["|\'])(.*?)\1@is', [$this, 'html_attr'], $str);
        ...
        // form
        $str = preg_replace_callback('@<form[^>]*action=(["\'])(.*?)\1[^>]*>@i', [$this, 'form_action'], $str);
